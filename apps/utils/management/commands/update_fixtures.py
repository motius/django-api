...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.write_fixtures(
            os.path.join(settings.BASE_DIR, 'apps/user/fixtures/user_data.json'),
            *['auth.user', 'auth.group', 'account']
        )

        self.write_fixtures(
            os.path.join(settings.BASE_DIR, 'apps/core/fixtures/initial_data.json'),
            *['sites']
        )
    ...
    def write_fixtures(self, filename, *args):
        ...
        call_command('dumpdata', *args, stdout=out, use_natural_primary_keys=True, use_natural_foreign_keys=True)
